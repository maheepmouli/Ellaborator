...
import {
  filterTrikalaEvidenceByPilot,
  loadTrikalaEvidenceManifest,
  type TrikalaEvidenceEntry,
} from "@/data/trikalaEvidenceManifest";
import { FileText, ImageIcon, ExternalLink } from "lucide-react";

interface TrikalaEvidencePanelProps {
  pilotId?: string | null;
}
  ...
  if (entry.type === "insight" || entry.type === "narrative" || (!entry.path && entry.fallback)) {
    return (
      <div className="rounded-lg border border-white/10 bg-white/5 p-3">
        <p className="text-[11px] font-medium text-white/90">{entry.title}</p>
        {entry.metric && (
          <p className="mt-1 text-[13px] font-semibold text-[#96c2ef]">{entry.metric}</p>
        )}
        <p className="mt-1.5 text-[11px] leading-relaxed text-white/65">
          {entry.fallback?.text ?? entry.caption ?? "Survey-derived narrative from SharePoint workbooks."}
        </p>
        {entry.linkedDatasetIds.length > 0 && (
          <p className="mt-2 text-[10px] text-white/45">
            Datasets: {entry.linkedDatasetIds.join(" · ")}
...
export function TrikalaEvidencePanel({ pilotId }: TrikalaEvidencePanelProps) {
  const [entries, setEntries] = useState<TrikalaEvidenceEntry[]>([]);

  useEffect(() => {
    void loadTrikalaEvidenceManifest().then((all) => {
      setEntries(filterTrikalaEvidenceByPilot(all, pilotId));
    });
  }, [pilotId]);
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] uppercase tracking-wide text-white/50">
        Survey evidence, partner docs &amp; environmental monitoring
      </p>
