...
import {
  filterIssyEvidenceByPilot,
  loadIssyEvidenceManifest,
  type IssyEvidenceEntry,
} from "@/data/issyEvidenceManifest";
import { FileText, ImageIcon, ExternalLink } from "lucide-react";

interface IssyEvidencePanelProps {
  pilotId?: string | null;
}

function EvidenceBlock({ entry }: { entry: IssyEvidenceEntry }) {
    ...
    return (
      <div className="rounded-lg border border-white/10 bg-white/5 p-3">
        ...
        <p className="mt-1.5 text-[11px] leading-relaxed text-white/65">
          {entry.fallback?.text ?? entry.caption ?? "SharePoint catalogue entry."}
        </p>
...
export function IssyEvidencePanel({ pilotId }: IssyEvidencePanelProps) {
  const [entries, setEntries] = useState<IssyEvidenceEntry[]>([]);

  useEffect(() => {
    void loadIssyEvidenceManifest().then((all) => {
      setEntries(filterIssyEvidenceByPilot(all, pilotId));
    });
  }, [pilotId]);
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] uppercase tracking-wide text-white/50">
        SharePoint evidence &amp; partner docs
      </p>
