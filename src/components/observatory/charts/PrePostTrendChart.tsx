...
export function PrePostTrendChart({ payload, compact }: PrePostTrendChartProps) {
  const linked = resolveBeforeAfter(payload);
  const isMock = !linked || payload.dataClass === "mock";
  const pair =
    linked ??
    ({
      before: Math.max(20, (Number(payload.kpiValue) || 48) - 4),
      after: Math.max(22, Number(payload.kpiValue) || 52),
      label: "Before / after (mock)",
    } as const);
