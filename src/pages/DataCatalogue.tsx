...
const DATA_TYPE_COLORS: Record<DataType, string> = {
  ...
  derived: "bg-blue-500/15 text-blue-300 border-blue-500/30",
  modelled: "bg-amber-500/15 text-amber-300 border-amber-500/30",
  mock: "bg-white/5 text-white/40 border-white/10",
};
...
function ParserBadge({ status }: { status: ParserStatus }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${PARSER_COLORS[status]}`}>
      {status}
    </span>
  );
}
...
function RealDataBadge({ status }: { status: RealDataStatus }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${REAL_DATA_COLORS[status]}`}>
...
function GeoQualityBadge({ quality }: { quality: GeometryQuality }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${GEO_QUALITY_COLORS[quality]}`}>
      {quality}
    </span>
  );
}
...
function DataTypeBadge({ type }: { type: DataType }) {
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${DATA_TYPE_COLORS[type]}`}>
      {type}
    </span>
  );
}
...
function DatasetRow({ dataset }: { dataset: DatasetMetadata }) {
  ...
  return (
    <>
      <TableRow
        className="cursor-pointer hover:bg-white/[0.035] transition-colors border-white/5 group"
        onClick={() => setOpen((v) => !v)}
      >
        ...
        <TableCell className="py-3">
          <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${dataset.beforeAfterStatus === "both" ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/30" : dataset.beforeAfterStatus === "before-only" || dataset.beforeAfterStatus === "after-only" ? "bg-amber-500/15 text-amber-300 border-amber-500/30" : "bg-white/5 text-white/40 border-white/10"}`}>
                  ...
                  return (
                    <td key={kpi.id} className="py-3 px-2 text-center">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span
                            className={`inline-flex items-center justify-center gap-1 rounded-lg border px-2 py-1 cursor-default font-medium ${READINESS_COLORS[cell.readiness]}`}
                          >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-2 rounded-xl border border-white/8 bg-white/[0.03] p-4 text-[11px] text-white/50 leading-relaxed">
        ...
        Datasets without geometry can still contribute to panel charts and KPI summaries but cannot render on the map.
        Segment joins and polygon assignments require a shared key (segment ID, zone ID) between the geometry file and the value table.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4">
      ...
      <Select value={filters.kpi} onValueChange={set("kpi")}>
        ...
        <SelectContent>
          <SelectItem value={FILTER_ALL}>All KPIs</SelectItem>
      ...
      <Select value={filters.geoQuality} onValueChange={set("geoQuality")}>
        ...
        <SelectContent>
          <SelectItem value={FILTER_ALL}>All geometry</SelectItem>
      ...
      <Select value={filters.parserStatus} onValueChange={set("parserStatus")}>
        ...
        <SelectContent>
          <SelectItem value={FILTER_ALL}>All parsers</SelectItem>
          ...
          <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-[14px] text-white/45 max-w-xl">
                Transparent metadata registry for all city datasets — sources, geometry quality,
                KPI linkage, and integration status in one place.
              </p>
            </div>
            <Button
              variant="outline"
          ...
          <GlassCard className="p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="text-[12px] text-white/50 leading-relaxed">
                <span className="text-white/70 font-medium">Marketplace integration roadmap. </span>
                Datasets currently loaded from SharePoint files and city APIs. Phase 3 will route all
                data through the Marketplace metadata API: SharePoint raw files → Marketplace metadata
                layer → ELABORATOR parser → normalised KPI engine → map + dashboard. GeoJSON
                intervention boundaries will be consumed from the Marketplace rather than drawn locally.
                This registry is pre-structured to map directly into that API response schema.
              </div>
            </div>
          </GlassCard>
        </motion.div>
